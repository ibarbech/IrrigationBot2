...
keys = ['/DectiveKeyboardOptions', '/ActiveKeyboardOptions', '/ProcessAllData', '/GetGraphics', '/unsubcribe',
...
def handle_start_help(message):
    if message.from_user.id not in subcribe.keys():
...
def handle_GetGraphics(message):
    Timer(1, lambda: SendGraphics(message)).start()
